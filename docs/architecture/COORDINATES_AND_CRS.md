...
invented from a GeoJSON document that does not define them. The direct-read
stage policy authors `/Vector` as the default prim and leaves both stage values
unset unless the `upAxis` or `metersPerUnit` FileFormat argument requests one.
Those arguments declare stage values only; they do not swap source axes,
reproject, or convert source units.
